export const docsZhCN = {
  ...
  features: [
    {
      icon: '🎨',
      title: '富文本编辑',
      desc: '基于 Quill 2.0，支持完整的富文本编辑功能，双击节点即可进入内联编辑'
    },
    {
      icon: '📐',
      title: '文本样式',
      desc: '字体、大小、颜色、对齐、行高、字间距、阴影、描边等全面控制'
    },
    {
      icon: '📝',
      title: '格式化工具',
    ...
    },
    {
      icon: '🔤',
      title: '自定义与多字体',
      desc: 'Base64 字体注入、多 @font-face、内嵌编辑下选区换字体'
    },
    {
      icon: '🔡',
      title: '局部样式',
    ...
    },
    {
      icon: '🎡',
      title: '弧形文字',
      desc: '基于 SVG TextPath 将文字沿弧形路径排列'
    },
    {
      icon: '🔍',
      title: '画布缩放',
      desc: '内置 zoom API：放大 / 缩小 / 适合屏幕 / 1:1'
    },
    {
      icon: '🔒',
      title: '锁定比例',
      desc: '选中元素后启用，拖拽缩放时保持宽高比不变'
    },
    {
      icon: '🔄',
      title: '旋转 & 缩放',
      desc: '通过 rotation / scaleX / scaleY 属性控制元素变换'
    },
    {
      icon: '🎯',
      title: '完美集成',
      desc: '无缝集成到 Leafer UI 生态系统，支持多选批量编辑'
    },
    {
      icon: '🔐',
      title: '授权管理',
      desc: '内置授权系统，本地开发不限制，生产环境需授权'
    },
    {
      icon: '🔧',
      title: 'TypeScript',
      desc: '完整的类型定义支持，ESM + CJS 双格式构建'
    }
  ],
  initLayers: [
    {
    ...
    },
    {
      ...
      params: [
        ...
        },
        {
          ...
          type: 'string',
          default: '—',
          desc: 'CSS -webkit-text-stroke，如 "2px #000"'
        },
        {
          name: 'align',
  ...
  setHtmlTextKeys: [
    ...
    },
    {
      key: 'underline',
      valueType: 'boolean（可省略）',
      desc: '下划线。内嵌编辑时改字符；外框选中时作用于全文'
    },
    {
      key: 'strike',
      valueType: 'boolean（可省略）',
      desc: '删除线。规则同 underline'
    },
    {
      key: 'textCase',
      valueType: '省略 value',
  ...
  manageMethods: [
      ...
      params: [
        {
          ...
          type: 'App',
          required: true,
          desc: 'Leafer App 实例（通过 new App() 创建）'
        }
      ],
      returns: {
        type: 'Promise<Quill>',
        desc: '返回初始化完成的 Quill 编辑器实例'
      },
    ...
    },
    {
      ...
      returns: {
        type: 'Quill | null',
        desc: '已初始化的 Quill 实例，授权失败时返回 null'
      },
    ...
    },
    {
      ...
      params: [
        ...
        },
        {
          ...
          type: 'number',
          required: false,
          desc: '操作层级，传 1 时会深入 Box 子节点操作内部 Text 元素'
        },
        {
          name: 'listNew',
          type: 'any[]',
          required: false,
          desc: '自定义节点列表，不传则使用当前编辑器选中列表'
        }
      ],
...
onMounted(async () => {
  ...
  app.tree.add(text)
    ...
    initScenarioContentCode: `// 场景 A：新建简单文本（推荐入门）
const text = new HtmlText({
  x: 120,
  y: 80,
  content: '双击此处编辑',
...
app.tree.add(text)`,
    initScenarioFullStyleCode: `// 场景 B：新建带完整样式 + 自定义字体
...
app.tree.add(text)`,
    initScenarioTextCode: `// 场景 C：直接传入完整 HTML（业务已保存富文本）
...
app.tree.add(text)`,
    initScenarioJsonCode: `// 场景 D：Leafer JSON 回显（推荐纯 Leafer 项目）
...
existing?.set(saved.data)`,
    initScenarioSplitStorageCode: `// 场景 E：字体与内容分开存储
...
new HtmlText({
  text: fontStyle + contentHtml,
    ...
    multiFontStorageSchemaExample: `// 业务库 / 接口返回示例（插件不解析这些字段，由你的项目约定）
{
  "id": "text_001",
...
app.tree.add(text)`,
    setHtmlTextExample: `// 1. 基础文字格式（编辑模式下对选区生效）
...
canvas?.editor?.cancel()`,
  ...
  htmlTextManage.dateEdit((leaf) => {
    ...
    manageDateEditExample: `// 批量修改字体大小
htmlTextManage.dateEdit((leaf) => {
  leaf.fontSize = 24
}, 1)
  ...
  ui: {
    shared: {
      tableHeaders: {
        ...
        howToPass: '怎么传',
        phase: '阶段',
        businessStore: '业务库存什么',
        passToPlugin: '传给插件什么',
        yourBusinessData: '你的业务数据',
        whatToSave: '保存什么',
        howToRestore: '回显怎么传',
        fitLevel: '适合程度',
        resourceData: '资源 / 数据',
        ...
        checkItem: '检查项',
        whyImportant: '为什么重要',
        recommendedAction: '推荐做法',
        name: '名称',
        meaning: '含义',
        commonMisuse: '常见误用',
        key: 'key',
        valueType: 'value 类型 / 可选值',
        scenario: '场景',
        howYouOperate: '你怎么操作',
        apiEffect: '多数 API 的效果',
        whatYouWant: '你想做什么',
        needInnerEdit: '要不要双击编辑',
        needSelection: '要不要拖选文字',
        invoke: '调用',
        outerSelected: '① 外框选中',
        innerNoSelection: `② 内嵌编辑
无拖选`,
        innerWithSelection: `③ 内嵌编辑
有拖选`,
        inlineBehavior: '行为',
    ...
    introduction: {
      ...
      npmPackage: 'npm 包',
      githubSource: 'GitHub示例源码',
      liveDemo: '在线演示',
      lead: '一个强大的',
      leadStrong: 'Leafer UI',
      leadSuffix: '富文本编辑器插件，集成 Quill 2.0，支持完整的 HTML 文本编辑和丰富的文本样式控制。',
      communityTitle: '接入遇到问题？',
      communityDesc: '去社区反馈 Bug、提交功能建议，或者查看其他用户的讨论。',
      communityLink: '去社区讨论'
    },
    installation: {
      title: '安装',
    ...
    quickStart: {
      title: '快速开始',
      viteTitle: 'Vite 项目配置',
      viteDesc: '为确保插件正常工作，需在 Vite 配置中添加以下设置，避免多实例冲突：',
      webpackTitle: 'Webpack 项目配置'
    },
    initGuide: {
      title: '初始化指南',
      intro: '接入插件时需要理解',
      introTwoLayers: '两层初始化',
      introMiddle: '：先在页面里挂载编辑器能力，再按业务场景创建',
      introSuffix: '节点。两层都完成后，双击编辑、样式工具栏、保存回显才会正常工作。',
      layer1Title: '第一层：页面级初始化',
      layer1Desc: '每个页面只需执行一次。顺序固定：',
      layer1FlowMiddle: '→ 创建',
      layer1Suffix: '。',
      licenseCalloutTitle: '授权顺序：',
      licenseCallout: '必须在',
      licenseCalloutBefore: '之前调用。本地',
      licenseCalloutDev: '开发不受限；生产域名需有效 License。',
      businessQuestionTitle: '先想清楚：你在接哪一种业务？',
      ...
      priorityRules: [
        ...
        '传 text 时，插件会从 HTML 反解 fontSize 、 fontFamily 、 padding 等元数据；但你显式传入的参数优先级更高',
        '只传 content 时，插件会按 fontSize 、 color 、 align 等样式参数自动拼出完整 HTML',
        '既不传 content 也不传 text 时，会生成默认占位文本 Hello World'
      ],
      dontMixTitle: '不要混用的几件事',
      scenariosTitle: '按场景初始化',
      scenariosDesc: '涉及多种自定义字体回显时，先看',
      scenariosLink: '多字体与回显',
      scenariosSuffix: '专章。',
      paramsTitle: '构造参数一览',
      paramsDesc: '接受',
      paramsMiddle: '，即插件文本属性 + Leafer',
      paramsSuffix: '通用属性。以下按用途分组，完整字段表见',
    ...
    dataPersistence: {
      title: '数据保存与回显',
      intro: '本质是 Leafer 画布节点。保存与回显时，优先选择与',
      introLink: '初始化方式',
      introSuffix: '对应的方案即可，不必重复拼 HTML。',
      chooseTitle: '先选存储方案',
      chooseDesc1: '如果不确定怎么存，',
      chooseSingle: '单字体',
      chooseSingleAction: '可优先 toJSON() 。',
      chooseMulti: '多字体且含大体积中文字库',
      chooseMultiAction: '时，请用',
      chooseMultiLink: '多字体与回显',
      chooseMultiSuffix:
        '中的拆分存储 + hydrate 约定，不要把完整 base64 长期塞进 JSON 或数据库。 toJSON() 会把外层位置、宽高、内层 HTML、局部样式等一起保存；回显时 new HtmlText(json) 即可，但存盘前建议按业务需要做字体瘦身。',
      whySplitTitle: '为什么要拆分存储？',
      whySplitDesc:
        'fontStyle + contentHtml 不是插件强制的数据格式，而是业务上常见的轻量存储方案 ：字体 base64 往往很大，如果每个文本节点都把完整字体写进数据库，会造成大量重复。更合理的做法是字体、图片等大资源单独入库或走 CDN，文本节点只保存引用和必要的 HTML 样式；回显时再把资源拼回 text 传给 HtmlText 。',
      judgeCalloutTitle: '判断原则：',
      judgeCallout:
      ...
      mixedDesc:
        '如果画布只是页面的一部分，或者文本需要跟业务表单、素材库、模板系统分别加载，建议只保存每个 HtmlText 的 JSON。回显时每条文本数据都应该创建一个独立节点实例，再添加到画布；如果要更新已有节点，则找到节点后调用 set(textJson) 同步属性。',
      updateTitle: '更新已有节点：三种常见方式',
      updateDesc:
        '回显不是只有 new HtmlText() 。如果画布上已经有文本节点，需要按你更新的是「整份节点」「纯内容」还是「工具栏样式」选择不同入口。',
      localStyleTitle: '局部样式后的保存与回显',
      localStyleDesc:
      ...
      multiFontHtmlDesc:
        '一段文本可同时嵌入多个自定义字体。保存时 HTML 的 <style> 里会有多条 @font-face ；局部字体还会出现在 <span style="font-family:..."> 。只传 text 初始化或回显时，插件会通过 parseHtmlTextData 反解基础样式；进入编辑时也会把 HTML 里的 @font-face 注入到编辑器环境 。',
      multiFontHtmlItems: [
        '推荐：保存 HtmlText.toJSON() 或整棵 Leafer JSON，可同时保留节点位置、textData 与内部 HTML',
        '只存 HTML：必须保留完整 <style> （含所有 @font-face ）和内容 <span> 样式',
    ...
    editingGuide: {
      title: '编辑原理与常见问题',
      intro: '很多「进不了编辑态」「Quill 没反应」「openInnerEditor 无效」的问题，并不是插件坏了，而是',
      introLeafer: 'Leafer 编辑器',
      introAnd: '和',
      introQuill: 'Quill 内嵌编辑器',
      introSuffix: '是两套机制，需要先理解它们的协作关系，再写自定义交互。',
      layersTitle: '三层结构：画布节点、渲染层、编辑层',
      layersDesc: '一个可编辑的富文本，在运行时其实同时存在三层：',
      layers: [
        "外层 HtmlText （Leafer Box）— 你在画布上创建、拖拽、缩放的对象。默认 editOuter: 'TextEditTool' ，单击后进入「外框编辑态」。",
        "内层 HTMLText （Leafer HTML 渲染）— HtmlText 自动创建的子节点，负责把 HTML 画到画布上。它带有 editInner: 'TextEditor' ，是进入 Quill 编辑时的真正目标。",
        'DOM 层 #textInnerEditor （Quill 容器）— 插件在 document.body 上创建的固定定位 div，双击后显示，承载 Quill 输入。编辑结束会写回内层 HTMLText 的 text 字段。'
      ],
      mnemonicTitle: '记忆口诀：',
      mnemonic: '画布上看的是 HtmlText ，富文本内容存在 HTMLText.text ，真正打字的是页面上的 #textInnerEditor 。',
      interactionTitle: '默认交互：单击选中，双击编辑',
      interactionDesc: '插件内置的默认行为如下，和常见设计工具一致：',
      interactionItems: [
        '单击 文本框 → Leafer Editor 选中 HtmlText → 加载外框工具 TextEditTool （可拖拽、缩放、旋转）',
        '双击 已选中的文本框 → TextEditTool 监听 PointerEvent.DOUBLE_TAP → 调用 editor.openInnerEditor(htmlText, true) → 打开 TextEditor 内嵌编辑器',
        '在内嵌编辑态点击画布空白处 → 关闭内嵌编辑器，内容同步回画布'
      ],
      namingTitle: '最容易混淆的三个名字',
      namingRows: [
        {
          name: 'TextEditTool',
          type: '外框编辑工具',
          meaning: 'HtmlText.editOuter 的默认值。负责选中框、缩放、旋转，监听双击事件。',
          misuse: '把它当成内嵌编辑器名称传给 openInnerEditor'
        },
        {
          name: 'TextEditor',
          type: '内嵌编辑器',
          meaning: 'HTMLText.editInner 的默认值。负责加载 Quill、同步样式、写回 HTML。',
          misuse: '写成 textInnerEditor 或其他自定义字符串'
        },
        {
          name: 'textInnerEditor',
          type: 'DOM 元素 id',
          meaning: "Quill 挂载的 div 容器，可用 document.querySelector('#textInnerEditor') 找到。",
          misuse: "误传给 openInnerEditor(node, 'textInnerEditor') — 这不会打开编辑器"
        }
      ],
      namingWarningTitle: '重点：',
      namingWarning: 'textInnerEditor 是页面 DOM 的 id； TextEditor 才是 Leafer 内嵌编辑器注册名。两者完全不同。',
      openInnerTitle: '程序化打开内嵌编辑器',
      openInnerDesc:
        "如果你想把「双击进入编辑」改成「单击进入编辑」，不要直接照抄网上片段里的 openInnerEditor(node, 'textInnerEditor', true) 。应按插件内部双击逻辑来写：",
      openInnerBadge: '正确示例',
      openInnerPointsTitle: '上面这段和插件源码里的双击逻辑一致，关键点是：',
      openInnerPoints: [
        '操作对象是内层 HTMLText ，不是外层 HtmlText',
        'openInnerEditor(htmlText, true) 的第二个参数是 boolean ，表示「同时选中该目标」，不是编辑器名称',
        '若节点尚未处于外框编辑态，需先 app.editor.target = htmlTextBox ，再打开内嵌编辑器',
        'htmlTextManage.getQuill() 和 setSelection 应等内嵌编辑器打开后再调用'
      ],
      quillTitle: 'Quill 什么时候可用？',
      quillDesc:
        'htmlTextManage.init(app) 会创建全局唯一的 Quill 实例，但它只有在 TextEditor 内嵌编辑器 load() 之后，才真正进入可编辑状态：',
      quillItems: [
        '仅选中、未双击 → 可以调用 setHTMLText() 改样式，但看不到 Quill 光标',
        '已进入内嵌编辑态 → 可以 getQuill() 、监听 text-change 、 setSelection()',
        '关闭内嵌编辑器后 → Quill 内容会写回 HTMLText.text ，画布重新渲染'
      ],
      faqTitle: '常见问题排查'
    },
    apiHtmlText: {
      title: 'API · HtmlText',
      intro:
        '继承自 Leafer UI 的 Box ，是富文本节点的核心类。 通过 @registerUI() 注册到 Leafer 系统，可直接添加到画布。',
      paramsTitle: '构造参数',
      paramsSubtitle: '所有参数均为可选，以下为完整参数列表：',
      exampleTitle: '示例'
    },
    apiManage: {
    ...
    apiSetHtml: {
      ...
      intro: '是应用文本样式的核心函数。多选时批量应用到所有选中节点。 使用前请先读',
      introLink: '局部与全局样式',
      ...
      setHtmlParams: [
        {
          ...
          type: 'string',
          required: true,
          desc: '样式键名，见下方 key 列表'
        },
        {
          name: 'value',
          type: 'any',
          required: false,
          desc: '样式值，不同 key 接受不同类型的值'
        },
        {
          name: 'base64font',
          type: 'string',
          required: false,
          desc: '仅 key 为 "font" 时使用，传入 Base64 格式的字体数据（data:font/woff2;base64,...）'
        }
      ],
      keysTitle: '支持的 key 列表',
      exampleTitle: '示例'
    },
    apiExperimental: {
      title: 'API · 局部与全局样式',
      intro: '是改样式的 API。搞懂下面两件事就不会乱：',
      steps: [
        '先选中 画布上的 HtmlText 节点（出现外框）。',
        '改「某几个字」时：双击进入内嵌编辑 → 用鼠标拖选一段字 （光标闪烁不算选区）。'
      ],
      scenariosTitle: '三种操作场景（决定改局部还是改整段）',
      pitfallsTitle: '常见踩坑：',
      pitfalls: [
        '只单击选中节点、没双击 → 只能改「整段全局」类属性（fontSize、lineHeight 等）',
        '双击进了编辑但只闪光标、没拖蓝字 → 多数字符格式会作用在全文 ，不是局部',
        'inlineFontSize 、局部 font 、局部 letterSpacing 必须有拖选的一段字 才生效',
        '内嵌编辑里全选 再改 font → 按整段全局字体处理，不是局部'
      ],
      cookbookTitle: '按你的目标查：该用哪个 API？',
      matrixTitle: '技术细节：各 key 在三种场景下的行为',
      matrixDesc: '给需要对照源码的同学。列标题对应上面的三种场景。',
      multiSelectCalloutTitle: '多选节点：',
      multiSelectCallout:
        '会对每个选中节点执行；但带「拖选」的局部样式只作用于当前内嵌编辑器里的 Quill 选区，不能一次给多个节点里的某几个字批量设样式。',
      inlineFontTitle: '局部字体与多字体（font）',
      inlineFontDesc:
        'setHTMLText(\'font\', fontFamily, base64font) 默认开启，无需 setFeatures 。 内嵌编辑且有选区时，只给选中文字加 span style="font-family:..." ；无选区或全选时改全局 textData.fontFamily 并清除局部字体。自定义字体请传第三个参数 base64font ，插件会写入内部 HTML 的 <style> 中，并在需要时合并多个 @font-face 。',
    ...
    apiLicense: {
      title: 'API · 授权管理',
      callout: '本地开发环境（ localhost ）不受授权限制，可自由使用全部功能。 部署到生产域名时需购买授权密钥。'
    },
    changelog: {
      title: '更新日志'
    }
...
export type DocsZhCN = typeof docsZhCN;
